/** Corpo do e-mail de conferência de medição (espelha src/lib/emailCorpoMedicaoCliente.ts). */
...
/** Corpo customizado (texto plano da UI) → HTML do e-mail. */
export function textoPlanoParaHtmlCorpoMedicao(texto: string): string {
  const t = texto.trim();
  if (!t) return montarHtmlCorpoMedicao();
...
export function montarHtmlCorpoMedicao(mesReferencia?: string | null): string {
  const mes = escapeHtml(
    (mesReferencia ?? "").trim() || formatarMesReferenciaMedicao(),
  );
  return `
