...
BEGIN
  ...
  SELECT c.peso_liquido INTO v_gravado
  FROM public.coletas c
  WHERE c.id = p_coleta_id;

  IF v_gravado IS NULL OR abs(v_gravado - p_peso_liquido) > 0.01 THEN
    RETURN jsonb_build_object(
      'ok',
      false,
      'message',
  ...
  RETURN jsonb_build_object('ok', true, 'peso_liquido', v_gravado);
END;
$$;
